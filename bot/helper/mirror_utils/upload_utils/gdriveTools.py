...
class GoogleDriveHelper:
    ...
    def __upload_file(self, file_path, file_name, mime_type, parent_id):
        ...
        file_metadata = {
            'name': file_name,
            'description': 'Uploaded by QMirrorLeech',
            'mimeType': mime_type,
        }
    ...
    def count(self, link):
        ...
        try:
            ...
            if mime_type == self.__G_DRIVE_DIR_MIME_TYPE:
                ...
                msg += f'\n<b>Size: </b><code>{get_readable_file_size(self.__total_bytes)}</code>'
                msg += '\n<b>Type: </b><code>Folder</code>'
                msg += f'\n<b>SubFolders: </b><code>{self.__total_folders}</code>'
                msg += f'\n<b>Files: </b><code>{self.__total_files}</code>'
            else:
                msg += f'<b>Name: </b><code>{name}</code>'
                if mime_type is None:
                    mime_type = 'File'
